...
# Initialize engine and mixer
pygame.init()
pygame.mixer.init()

# Setup main window and timing
...
def main_menu():
    # Build buttons for the starting screen
    buttons = {
...
def leaderboard_screen():
    # Create a simple list view for global rankings
    ...
    while True:
        ...
        # Try to pull rows from Postgres
        try:
            rows = get_top_scores()
...
def settings_screen(settings):
    # UI elements for user customization
    ...
    colors = [[0, 200, 0], [0, 120, 255], [220, 50, 50], [180, 0, 220], [255, 180, 0]]
    ...
    while True:
        ...
        # Visual preview of the selected color
...
def game_over_screen(username, score, level):
    # Display final stats and options after death
...
def play_game(settings):
    # Routine to handle player input for name and then start the game loop
    ...
    while True:
        ...
        # Attempt to log stats to the database
        try:
...
def main():
    # Application entry point: set up DB and load preferences
    try:
        create_tables()
    except Exception as error:
    ...
    # Master loop for switching between screens
    while True:
        action = main_menu()
        if action == "quit":
            break
        elif action == "play":
            if play_game(settings) == "quit":
                break
        elif action == "leaderboard":
            if leaderboard_screen() == "quit":
                break
        elif action == "settings":
            if settings_screen(settings) == "quit":
                break
